...
if (!function_exists('import_failed')) {
    function import_failed($file, $failures, $fileName)
    {
        $rowsData = [];
        $rowIndexs = [];
        $errors = [];
        ...
        $extension = getFileExtention($file);
        $reader = $extension == 'xlsx' ? PhpOffice\PhpSpreadsheet\IOFactory::createReader("Xlsx") : new \PhpOffice\PhpSpreadsheet\Reader\Csv();
        ...
        $pathToFile = storage_path('/app/public/'.$fileName.auth()->user()->id.'.xlsx');
        ...
        return [
            ...
            'fileName' => getImageUrl($fileName.auth()->user()->id.'.xlsx', '', false),
        ];
    }
}
