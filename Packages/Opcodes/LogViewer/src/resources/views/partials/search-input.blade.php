...
<script @if( config('log-viewer.enabled_nonce') ) nonce="{{csp_nonce('script')}}" @endif  wire:key="{{ 'search-key-'.md5($query) }}" x-init='$store.search.update(@json($query), @json($queryError), @json(route('blv.search-more')), @json($hasMoreResults), @json($percentScanned))'>
</script>
