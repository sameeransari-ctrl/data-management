...
let products = {
    ...
    addRemoveFileURL: function(){
        ...
        $(document).on('click', '.add_button', function() {
            var fieldHTML = `<div class="form-control-wrap field_wrapper mt-2"><div><div class="input-group copy-link"><input type="text" class="form-control shadow-none copy-link-input" name="file_url[]" placeholder="Add File URL" value="" aria-describedby="file_url-${x}-error"> <div class="input-group-append"><span class="input-group-text cursor-pointer copy-link-button" id="copyUrlText2"><em class="icon ni ni-link fs-14px"></em></span></div><a class="remove_button addMoreBtn" title="Add field"><em class="icon ni ni-minus-sm"></em></a></div><span id="file_url-${x}-error" class="help-block error-help-block"></span></div></div>`; //New input field html
        ...
        $(wrapper).on('click', '.remove_button', function(e){
            ...
            // Reset the index for remaining fields
            $(wrapper).find('.copy-link-input').each(function(index, input) {
                $(input).attr('aria-describedby', `file_url-${index}-error`);
            });

            // Update the IDs of the span elements
            $(wrapper).find('.help-block.error-help-block').each(function(index, span) {
                $(span).attr('id', `file_url-${index}-error`);
            });
        });
    },
    ...
    saveProduct:function(){
        $(document).on('click', '#productSubmitBtn', function(e) {
            ...
            if(productId=='') {
                ...
                url = route('client.product.update', productId);
                methodType = 'PUT';
                formData = frm.serialize() + "&id="+productId;
            }
            if (frm.valid()) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#questionForReviewSaveNContinueBtn').attr("data-product_id", productId);
    ...
    addQuestionModal: function(){
        $(document).on('click', '.addQuestionModal', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: route('client.product.add-question', questionType),
                beforeSend: function() {
                    //showButtonLoader(btn, btnName, true);
    ...
    editQuestionModal: function(){
        $(document).on('click', '.editQuestionModal', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: route('client.product.edit-question', questionId),
                beforeSend: function() {
                    //showButtonLoader(btn, btnName, true);
    ...
    questionRemove: function(){
        $(document).on('click', '.removeData', function(e) {
            ...
            let url = route('client.product.destroy-question',{id});
            Swal.fire({
    ...
    getProductQuestions: function() {
        $(document).on('click', '#questionForReviewSaveNContinueBtn', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: route('client.product.get-questions'),
    ...
    updateProductStatus: function() {
        $(document).on('click', '#productFinalSubmitBtn', function(e) {
            ...
            $.ajax({
                type: "PUT",
                url: route('client.product.update-product-status'),
