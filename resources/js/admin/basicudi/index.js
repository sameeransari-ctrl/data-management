let udi = {
    ...
    exportCsv: function () {
        $(document).on('click', '.exportCsv', function () {
            ...
            var url = route('admin.basicudi.export-csv');
    ...
    addEditModal: function () {
        $(document).on('click', '.addEditUdi', function () {
            ...
            let url = route('admin.basicudi.create');
            $.ajax({
                type: "GET",
                url: url,
                beforeSend: function () {
        ...
        NioApp.DataTable(userListTable, {
            ...
            'createdRow': function (row, data, full) {
                $('.edit-udi', row).on('click', function () {
                    $.ajax({
                        method: "GET",
                        url: route('admin.basicudi.edit', data),
                        success: function (response) {
                            $('#addUdi').html(response).modal('show');
                            $(".js-select2").select2();
                        },
    ...
    basicUdiModel: function () {
        $('#import-basic-udi').on('click', function(){
            ...
            $.ajax({
                type: 'GET',
                url: route('admin.basicudi.import-form'),
                beforeSend: function() {
                    showButtonLoader(btn, btnName, true);
                },
                success: function(response) {
...
$(function () {
    ...
    $(document).on('click', '#importBasicUdiBtn', function(){
        ...
        if (form.valid()) {
            ...
            $.ajax({
                type: 'POST',
                url: route('admin.basicudi.import'),
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
