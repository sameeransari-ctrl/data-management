<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{__('labels.add')}}</h5>
            <a href="#" class="close" data-bs-dismiss="modal" aria-label="Close">
                <em class="icon ni ni-cross"></em>
            </a>
        </div>
        <div class="modal-body">
            <form id="roleAddEditForm" action="{{ route('admin.role.store') }}" method="POST" enctype="multipart/form-data">
                <div class="row g-3">
                    <div class="col-lg-6">
                        <div class="form-group">
                        <label class="form-label">{{__('labels.role')}}</label>
                        <div class="form-control-wrap">
                            <input type="text" name="name" class="form-control shadow-none" placeholder="{{__('labels.enter')}} {{__('labels.role')}}" >
                        </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="accordion-item">
                                    <div class="moduleSec_title d-flex custom-checkText py-2">
                                        ...
                                        <div class="custom-control custom-control-sm custom-checkbox">
                                            ...
                                            <label class="custom-control-label fw-bold" for="customCheckAll_{{$i}}">{{ $menuName }}</label>
                                        </div>
                                    </div>
                                            ...
                                            <div class="row g-2 custom-checkText ml-30" id="allCheck_{{$i}}">
                                                @foreach($items as $key => $item)
                                                    <div class="col-lg-3 col-sm-6">
                                                        <div class="custom-control custom-control-sm custom-checkbox">
                                                            <input type="checkbox" name="permissions[]" value="{{$item->id}}" class="custom-control-input selectInput" id="customCheck_{{$menuId}}_{{$key}}"
                                                            aria-describedby="permissions-error" data-menuId="{{$i}}">
                                                            <label class="custom-control-label" for="customCheck_{{$menuId}}_{{$key}}">{{ $item->title }}</label>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                ...
                <div class="form-group text-end mt-lg-4 mt-2">
                    <button type="button" class="btn btn-primary me-1" id="roleSubmitBtn">{{__('labels.save')}}</button>
...
{!! JsValidator::formRequest('App\Http\Requests\Admin\RoleRequest', '#roleAddEditForm') !!}
