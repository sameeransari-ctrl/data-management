<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="addEditPreview"></h5>
            <a class="close" data-bs-dismiss="modal" aria-label="Close">
                <em class="icon ni ni-cross"></em>
            </a>
        </div>
        <div class="modal-body">
            <form id="fsnAddEditPreviewForm" action="{{ route('client.fsn.previewModal') }}" method="POST"
                ...
                <div class="row g-sm-3 g-2">
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <div class="form-control-wrap">
                                <input type="text" id="title" name="title" class="form-control shadow-none" value="@if(!empty($request)){{$request->title}}@else{{ old('title') }}@endif" placeholder="{{ __('labels.enter_title') }}">
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <select class="form-select js-select2 form-select2" data-placeholder="{{ __('labels.select_udi') }}" name="product_id"
                            id="productId" aria-describedby="product_id-error">
                                    <option></option>
                                @foreach ($products as $product)
                                    <option {{ !empty($request) && ($request->product_id == $product->id) ? 'selected' : '' }} value="{{ $product->id }}">{{ $product->udi_number }}</option>
                                @endforeach
                            </select>
                            <span id="product_id-error" class="help-block error-help-block"></span>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label class="form-label">{{ __('labels.product_name') }}</label>
                            <div class="form-control-wrap">
                                <input type="hidden" id="udiNumber" name="udiNumber">
                                <input type="text" id="productName" value="@if(!empty($request)){{$request->productName}}@else{{ old('productName') }}@endif" class="form-control shadow-none bg-light disabled-input" name="productName" placeholder="{{ __('labels.enter_product_name') }}" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12">
                        <div class="form-group">
                            <label class="form-label" for="description">{{ __('labels.notice_description') }}</label>
                            <div class="form-control-wrap">
                                <textarea class="form-control shadow-none" id="description" name="description" placeholder="{{ __('labels.write_description') }}">@if(!empty($request)){{$request->description}}@else{{ old('description') }}@endif</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label class="form-label">{{ __('labels.attach_type') }}</label>
                            <select class="form-select js-select2" name="attachType" id="selectType08" data-placeholder="{{ __('labels.attach_type') }}"
                                data-minimum-results-for-search="Infinity" aria-describedby="attachType-error">
                                <option></option>
                                <option {{ !empty($request) && ($request->attachType == 'xlsx') ? 'selected' : '' }} value="xlsx">{{ __('labels.xlsx') }}</option>
                                <option {{ !empty($request) && ($request->attachType == 'video') ? 'selected' : '' }} value="video">{{ __('labels.video') }}</option>
                                <option {{ !empty($request) && ($request->attachType == 'url') ? 'selected' : '' }} value="url">{{ __('labels.url') }}</option>
                            </select>
                            <span id="attachType-error" class="help-block error-help-block"></span>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="select-panel d-none" id="xlsx">
                            <div class="form-group">
                                <label class="form-label" for="customFileLabel">{{ __('labels.file_upload') }}</label>
                                <div class="form-control-wrap">
                                    <div class="form-file">
                                        <input type="file" accept="{{ config('constants.file.fsn.xlsx.acceptType') }}" name="xlsx_file" class="form-file-input cursor-pointer xlsx_file" id="uploadFile">
                                        <label class="form-file-label shadow-none" for="uploadFile">{{ __('labels.choose_file') }}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="select-panel d-none" id="video">
                            <div class="form-group">
                                <label class="form-label" for="customFileLabel">{{ __('labels.file_upload') }}</label>
                                <div class="form-control-wrap">
                                    <div class="form-file">
                                        <input type="file" accept="{{ config('constants.file.fsn.video.acceptType') }}" name="video_file" class="form-file-input cursor-pointer video_file" id="uploadFile">
                                        <label class="form-file-label shadow-none" for="uploadFile">{{ __('labels.choose_file') }}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="select-panel d-none" id="url">
                            <div class="form-group">
                                <label class="form-label" for="uploadUrl5">{{ __('labels.url') }}</label>
                                <div class="form-control-wrap">
                                    <div class="input-group copy-link">
                                        <input type="text" name="url" id="input-url" class="form-control shadow-none copy-link-input url"
                                            placeholder="{{ __('labels.url_link') }}">
                                        <div class="input-group-append">
                                            <span class="input-group-text copy-link-button cursor-pointer"
                                                id="uploadUrl5"><em class="icon ni ni-link fs-14px"></em></span>
                                        </div>
                                    </div>
                                    <span id="input-url-error" class="help-block d-none error-help-block">{{ __('messages.error.invalid_url') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12">
                        <div class="form-group text-end">
                            <button type="button" class="btn btn-primary me-1" id="submitFsn">{{__('labels.submit')}}</button>
                            <button type="button" data-bs-dismiss="modal" class="btn btn btn-light" id="cancelBtn">{{ __('labels.cancel') }}</button>
                        </div>
                    </div>
                </div>
                </div>
                ...
                <div id="previewSection">
                    <div class="row g-sm-3 g-2">
                        <div class="col-sm-6">
                            ...
                            <p class="card-text text-break" id="title1"></p>
                        </div>
                        <div class="col-sm-6">
                            <h5 class="lead-text mb-2">{{ __('labels.udi_no') }}</h5>
                            <p class="card-text text-break" id="udiNumber1"></p>
                        </div>
                        <div class="col-sm-6">
                            <h5 class="lead-text mb-2">{{ __('labels.product_name') }}</h5>
                            <p class="card-text text-break" id="productName1"></p>
                        </div>
                        <div class="col-12">
                            <h5 class="lead-text mb-2">{{ __('labels.description') }}</h5>
                            <p class="card-text text-break mb-2" id="description1"></p>
                        </div>
                    </div>
                    <div class="upload-file">
                        ...
                        <ul class="d-flex g-2 flex-wrap">
                            <li id="xlsx1">
                                    <div class="preview-icon-box border rounded box-shadow p-2" id="xlsxtitle">
                            ...
                            <li id="video1">
                                    <div class="preview-icon-box border rounded box-shadow p-2" id="videotitle">
                                        <div class="preview-icon-wrap w-90px h-70 mx-auto">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 72" class="w-80px h-70">
                            ...
                            <li id="url1">
                                    <div class="preview-icon-box border rounded box-shadow p-2" id="urltitle">
                                        <div class="preview-icon-wrap w-90px h-70 mx-auto">
                        ...
                        {{-- <div class="mt-2 text-break">(examd.jpg)</div> --}}
                    </div>
                    <div class="text-end mt-3">
                        <button type="button" class="btn btn-primary me-1" id="confirmBtn">{{ __('labels.confirm_and_send') }}</button>
                        <button type="button" class="btn btn btn-outline-primary" id="editBtn">{{ __('labels.edit') }}</button>
                    </div>
                </div>
                <!-- end preview section -->
            </form>
...
{!! JsValidator::formRequest('App\Http\Requests\Client\FsnRequest', '#fsnAddEditPreviewForm') !!}
